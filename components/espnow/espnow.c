...
static void example_espnow_task(void *pvParameter)
{
    ...
    for (size_t i = 0; i < 4; ++i) {
        if (isprint(send_param->buffer[i])) {
            printf("%c", send_param->buffer[i]);
        } else {
            printf(".");
        }
    }
...
void espnow_send_timestamp(example_espnow_send_param_t *params, int64_t timestamp)
{
    example_espnow_event_t evt;
    uint8_t recv_state = 0;
    uint16_t recv_seq = 0;
    ...
    example_espnow_send_param_t *send_param = params;
